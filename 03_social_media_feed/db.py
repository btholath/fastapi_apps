users = {
    "alexjohnson": {
        "name": "Alex Johnson",
        "username": "alexjohnson",
        "email": "alex.johnson@example.com",
        "birthday": "5th February 1985",
        "friends": ["briansmith", "emilyjones", "michaelclark"],
        "notifications": [
            {
                "author": "briansmith",
                "description": "commented on your post."
            },
            {
                "author": "emilyjones",
                "description": "shared a photo with you."
            },
            {
                "author": "michaelclark",
                "description": "wished you a happy birthday!"
            },
        ],
        "hashed_password": "$2b$12$aE34bS5MzXpQzQVYn/PxieKH8d8JF9dmG9SKZP0flHF3jEiKYc71W",
        "password": "alex_password_123"
    },
    "briansmith": {
        "name": "Brian Smith",
        "username": "briansmith",
        "email": "brian.smith@example.com",
        "birthday": "20th July 1990",
        "friends": ["alexjohnson", "davidthomas"],
        "notifications": [
            {
                "author": "alexjohnson",
                "description": "liked your photo."
            },
            {
                "author": "davidthomas",
                "description": "mentioned you in a comment."
            },
            {
                "author": "emilyjones",
                "description": "sent you a message: \"Let's catch up soon!\""
            },
        ],
        "hashed_password": "$2b$12$7f.kDiIH3LjK3Q.mGfGfU.Drm7z4eqeJiX3kS0tC/hFLtL3h6.wFq",
        "password": "brian_password_456"
    },
    "emilyjones": {
        "name": "Emily Jones",
        "username": "emilyjones",
        "email": "emily.jones@example.com",
        "birthday": "10th March 1995",
        "friends": ["alexjohnson", "briansmith", "sarahwilson"],
        "notifications": [
            {
                "author": "alexjohnson",
                "description": "reacted to your story."
            },
            {
                "author": "briansmith",
                "description": "commented on your photo."
            },
            {
                "author": "sarahwilson",
                "description": "sent you a friend request."
            },
        ],
        "hashed_password": "$2b$12$C/5DFiH1HtOkf1fD54P5o.cuYxRYYGKrmTkYmYz.jfq/nU8W9Uz5K",
        "password": "emily_password_789"
    },
    "michaelclark": {
        "name": "Michael Clark",
        "username": "michaelclark",
        "email": "michael.clark@example.com",
        "birthday": "15th August 1988",
        "friends": ["alexjohnson", "sarahwilson"],
        "notifications": [
            {
                "author": "alexjohnson",
                "description": "shared a link with you."
            },
            {
                "author": "sarahwilson",
                "description": "liked your comment."
            },
            {
                "author": "briansmith",
                "description": "wished you a happy birthday!"
            },
        ],
        "hashed_password": "$2b$12$9s4kRj8JsTpX8yVkPh3iZfhNp9dDxML3s7QPBtMwHGIFAnJbL4ZqO",
        "password": "michael_password_123"
    },
    "sarahwilson": {
        "name": "Sarah Wilson",
        "username": "sarahwilson",
        "email": "sarah.wilson@example.com",
        "birthday": "30th September 1992",
        "friends": ["emilyjones", "michaelclark"],
        "notifications": [
            {
                "author": "emilyjones",
                "description": "commented on your photo."
            },
            {
                "author": "michaelclark",
                "description": "sent you a message: \"How are you?\""
            },
            {
                "author": "briansmith",
                "description": "tagged you in a post."
            },
        ],
        "hashed_password": "$2b$12$p2mQVJ7Sy8j4F.ZRG3p/lOHGJnUyYDKw4KIQcHr5OeBFT3lJN4X1G",
        "password": "sarah_password_456"
    },
    "davidthomas": {
        "name": "David Thomas",
        "username": "davidthomas",
        "email": "david.thomas@example.com",
        "birthday": "25th December 1987",
        "friends": ["briansmith", "sarahwilson"],
        "notifications": [
            {
                "author": "briansmith",
                "description": "shared a video with you."
            },
            {
                "author": "sarahwilson",
                "description": "liked your status update."
            },
            {
                "author": "alexjohnson",
                "description": "reacted to your post."
            },
        ],
        "hashed_password": "$2b$12$4Ns2pJ8aXk7yN3VYN/GfXsJmW7hF4eTkG6MkGnHz/iJDJ8mL6F6U",
        "password": "david_password_789"
    },
    "laurabrown": {
        "name": "Laura Brown",
        "username": "laurabrown",
        "email": "laura.brown@example.com",
        "birthday": "12th June 1993",
        "friends": ["alexjohnson", "emilyjones"],
        "notifications": [
            {
                "author": "alexjohnson",
                "description": "commented on your status."
            },
            {
                "author": "emilyjones",
                "description": "shared a post with you."
            },
            {
                "author": "briansmith",
                "description": "reacted to your comment."
            },
        ],
        "hashed_password": "$2b$12$W7lQeJ9GhG2yTmS4R7fM.XtVp8sK6N7u7OZJbM5LpF3nR5YT4X6yO",
        "password": "laura_password_123"
    },
    "robertmartin": {
        "name": "Robert Martin",
        "username": "robertmartin",
        "email": "robert.martin@example.com",
        "birthday": "3rd November 1980",
        "friends": ["alexjohnson", "michaelclark"],
        "notifications": [
            {
                "author": "michaelclark",
                "description": "shared a link with you."
            },
            {
                "author": "alexjohnson",
                "description": "liked your photo."
            },
            {
                "author": "briansmith",
                "description": "wished you a happy birthday!"
            },
        ],
        "hashed_password": "$2b$12$G6hQnT8jK2mX9PnZ2E5.Ot4KJ8kRbJ4F8sYkWpN2ZC7fFxYxK3XyR",
        "password": "robert_password_456"
    },
    "lisawilliams": {
        "name": "Lisa Williams",
        "username": "lisawilliams",
        "email": "lisa.williams@example.com",
        "birthday": "18th May 1994",
        "friends": ["sarahwilson", "emilyjones"],
        "notifications": [
            {
                "author": "sarahwilson",
                "description": "reacted to your story."
            },
            {
                "author": "emilyjones",
                "description": "commented on your photo."
            },
            {
                "author": "alexjohnson",
                "description": "sent you a message: \"Hey!\""
            },
        ],
        "hashed_password": "$2b$12$x8yQKJ7PrW6tNkLnM5D/JZsNh7uM6gF4tG4LkTpJz/iLD8bH6X4uQ",
        "password": "lisa_password_789"
    },
}